...
code = code.replace("const [selectedElementId, setSelectedElementId] = useState(null);", "const [selectedElementIds, setSelectedElementIds] = useState([]);")

# 4. REMOVE EXACT SELECTION BOX
...
if start_sel != -1 and end_sel != -1:
    ...
    print("Could not find selection box")
...
"""
code = code.replace("if (el.type === 'path') {", lasso_draw + "      if (el.type === 'path') {")
...
"""
code = code.replace("  }, [zoom, pan, selectedElementId]);", group_box_draw)
...
if start_pd != -1 and end_pd != -1:
    ...
    print("Could not find onPointerDown select")
...
if start_pm != -1 and end_pm != -1:
    ...
    print("Could not find onPointerMove select")
...
if start_pu != -1 and end_pu != -1:
    ...
    print("Could not find onPointerUp select")
